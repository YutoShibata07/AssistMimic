...
class HumanoidImHhiAssistAssistMimic(HumanoidIm, RSIMixin):
    """
    Humanoid Interaction class for single-humanoid per environment interaction tasks with bed/chair assets.
    Following the instruction: 1 environment = 1 humanoid, with collision groups for pair interaction.
    Includes bed/chair asset in recipient environments (based on humanoid_im_humanx_hhi_assist_bed.py).
    """
    ...
    def __init__(self, cfg, sim_params, physics_engine, device_type, device_id, headless):
        ...
        # Asset name configuration (bed or chair)
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        """Override to create single-humanoid environments with collision groups for interaction and bed/chair assets"""
    ...
    def _load_bed_asset(self):
        ...
        if self.asset_name == "chair":
            ...
            furniture_height = 0.508
            print(f"Loading chair asset: {furniture_length}m L x {furniture_width}m W x {furniture_height}m H")
        else:
            # Bed dimensions based on motion type
            if "AA-RM-wo-FullAssist" in self.hhi_assist_bed_data_path or "ED-GG-wo-FullAssist" in self.hhi_assist_bed_data_path or "GR-JW-wo-FullAssist" in self.hhi_assist_bed_data_path:
                furniture_length = 1.905
                furniture_width = 0.9906
                furniture_height = 0.4572
            elif "BC-CC-wo-FullAssist" in self.hhi_assist_bed_data_path or "JB-BC-wo-FullAssist" in self.hhi_assist_bed_data_path:
                furniture_length = 0.9906  # Swapped for these motions
                furniture_width = 1.905    # Swapped for these motions
                furniture_height = 0.4572
            else:
        ...
        # Create a single shared furniture asset
        ...
        # Create box with furniture dimensions
        if self.asset_name == "chair":
    ...
    def _build_bed_tensors(self):
        """Build bed-related tensors for all environments"""
        if not hasattr(self, '_root_states') or self._root_states is None:
            print("Warning: _root_states not initialized yet, skipping bed tensor build")
            return
        ...
        # Calculate bed actor IDs (flat indices)
    ...
    def _build_env(self, env_id, env_ptr, humanoid_asset):
        """Override to add bed spawning for all environments"""
        # Call parent method to build humanoid
        super()._build_env(env_id, env_ptr, humanoid_asset)

        # Spawn bed for all environments
        self._build_bed(env_id, env_ptr)

        return

    def _build_bed(self, env_id, env_ptr):
        """Build bed/chair in environment"""
        ...
        if env_id % 2 == 1:
            # Recipient environment: position bed properly with motion-specific adjustments
            # X position adjustment based on motion type
            if "AA-RM-wo-FullAssist" in self.hhi_assist_bed_data_path or "ED-GG-wo-FullAssist" in self.hhi_assist_bed_data_path:
                default_pose.p.x = - self.cfg["env"].get("env_spacing", 5.0) * 2 + 0.3
            elif "GR-JW-wo-FullAssist" in self.hhi_assist_bed_data_path:
                default_pose.p.x = - self.cfg["env"].get("env_spacing", 5.0) * 2 - 0.3
            elif "BC-CC-wo-FullAssist" in self.hhi_assist_bed_data_path:
                default_pose.p.x = - self.cfg["env"].get("env_spacing", 5.0) * 2 + 0.1
            elif "JB-BC-wo-FullAssist" in self.hhi_assist_bed_data_path:
                default_pose.p.x = - self.cfg["env"].get("env_spacing", 5.0) * 2 - 0.15
            else:
                default_pose.p.x = - self.cfg["env"].get("env_spacing", 5.0) * 2

            # Z position (height)
            default_pose.p.z = furniture_height / 2.0

            # Y position adjustment based on motion type
            if self.asset_name == "chair":
        ...
        if env_id % 2 == 0:
            # Caregiver: disable collisions
            disable_all = (1 << 16) - 1
            for sp in shape_props:
                sp.filter = disable_all
            self.gym.set_actor_rigid_shape_properties(env_ptr, target_handle, shape_props)
        else:
            # Recipient: enable collisions with proper settings
            for sp in shape_props:
            ...
            # Disable bed collisions with finger joints
            if hasattr(self, '_body_name_to_id'):
                humanoid_handle = self.humanoid_handles[env_id]
    ...
    def _setup_tensors(self):
        ...
        # Calculate actual num_actors from tensor size
        ...
        print(f"[DEBUG] bodies_per_env with beds: {bodies_per_env}")

        # Modified assertion for beds
        expected_bodies_options = [24, 48, 52, 104, 24 + 1, 48 + 1, 52 + 1, 104 + 1]
        assert bodies_per_env in expected_bodies_options, f"bodies_per_env should be one of {expected_bodies_options}, got {bodies_per_env}"
        ...
        # Initialize key body IDs and contact body IDs
    ...
    def _setup_character_props(self, key_bodies):
        ...
        # Filter out bed-related bodies
        humanoid_key_bodies = [body for body in key_bodies if 'bed' not in body.lower() and 'chair' not in body.lower()]

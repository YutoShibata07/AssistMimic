...
from assistmimic.utils.motion_lib_base import MotionLibBase, DeviceCache, compute_motion_dof_vels, FixHeightMode
...
class MotionLibSMPL(MotionLibBase):
    ...
    def fix_trans_height(pose_aa, trans, curr_gender_betas, mesh_parsers, fix_height_mode):
        ...
        with torch.no_grad():
            ...
            elif fix_height_mode == FixHeightMode.pene_fix:
                # Only fix penetration: if min height < 0, lift to 0. Otherwise, leave as-is (allow floating)
                ankle_ids = [mesh_parser.joint_names.index("L_Ankle"), mesh_parser.joint_names.index("R_Ankle")]
                ankle_min_height = ((vertices_curr[:, ankle_ids] )[:frame_check, ..., -1].min(dim=-1).values).min()
                head_height = ((vertices_curr[:, mesh_parser.joint_names.index("Head")] - offset[:, None])[:frame_check, ..., -1].min(dim=-1).values).min()
                if ankle_min_height > 0.30 and head_height < 0.80:
                    ankle_min_height = 0.6
                    diff_fix = ((vertices_curr[:, ankle_ids] - offset[:, None])[:frame_check, ..., -1].min(dim=-1).values - ankle_min_height).min()
                    # diff_fix = diff_fix if diff_fix < 0 else 0
                else:
                    # diff_fix = 0
                    min_height = ((vertices_curr - offset[:, None])[:frame_check, ..., -1].min(dim=-1).values - height_tolorance).min()
                    diff_fix = min_height if min_height < 0 else 0
            else:
                # For other cases including False, set diff_fix to 0
                diff_fix = 0
    ...
    def load_motion_with_skeleton(ids, motion_data_list, skeleton_trees, shape_params, mesh_parsers, config, queue, pid):
        ...
        for f in range(len(motion_data_list)):
            ...
            try:
                trans = curr_file['trans'].clone()[start:end]
            except:
                trans = curr_file['root_trans_offset'][start:end]
            ...
            # Skip heading randomization for motion files with "simpleliftup" or "interx" in name
            # to maintain consistent caregiver-recipient facing in SimpleLiftUp mode
            skip_heading_randomization = True
            
            if (not flags.im_eval) and (not flags.test) and (not skip_heading_randomization):
                # if True:

...
class FixHeightMode(Enum):
    ...
    full_fix = 1
    ankle_fix = 2
    pene_fix = 3  # Only fix penetration (min height < 0), allow floating
...
class MotionLibBase():
    ...
    def setup_constants(self, fix_height = FixHeightMode.full_fix, multi_thread = True):
        ...
        # ペア構造の初期化
        self._init_pair_structure()

    def _init_pair_structure(self):
        """ペア構造を初期化する"""
        self._motion_pairs = {}  # {unique_motion_key: [caregiver_idx, recipient_idx]}
        self._pair_indices = []  # ペアのインデックスリスト

        # モーションキーからペアを構築
        for i, key in enumerate(self._motion_data_keys):
            base_key = key.replace('_caregiver', '').replace('_recipient', '')
            if base_key not in self._motion_pairs:
                self._motion_pairs[base_key] = {}

            if '_caregiver' in key:
                self._motion_pairs[base_key]['caregiver'] = i
            elif '_recipient' in key:
                self._motion_pairs[base_key]['recipient'] = i

        # 完全なペアのみを保持
        complete_pairs = []
        for base_key, pair_dict in self._motion_pairs.items():
            if 'caregiver' in pair_dict and 'recipient' in pair_dict:
                complete_pairs.append([pair_dict['caregiver'], pair_dict['recipient']])
            else:
                error_msg = f"Incomplete pair found: {base_key} - {pair_dict}"
                raise ValueError(error_msg)

        self._pair_indices = torch.tensor(complete_pairs, device=self._device)
        self._num_pairs = len(complete_pairs)
        print(f"Found {self._num_pairs} complete motion pairs")

    def _get_pair_sampling_prob(self):
        """ペア単位のサンプリング確率を取得"""
        if self._num_pairs == 0:
            return torch.ones(1, device=self._device)

        # 各ペアの確率は構成するモーションの平均確率
        pair_probs = torch.zeros(self._num_pairs, device=self._device)
        for i, (caregiver_idx, recipient_idx) in enumerate(self._pair_indices):
            pair_probs[i] = (self._sampling_prob[caregiver_idx] + self._sampling_prob[recipient_idx]) / 2.0

        # 正規化
        return pair_probs / pair_probs.sum()

    def _get_paired_motion_idxes(self, pair_idxes):
        """ペアインデックスから対応するモーションインデックスを取得"""
        motion_idxes = []
        for pair_idx in pair_idxes:
            caregiver_idx, recipient_idx = self._pair_indices[pair_idx]
            motion_idxes.extend([caregiver_idx, recipient_idx])
        return torch.tensor(motion_idxes, device=self._device)
    ...
    def load_motions(self, skeleton_trees, gender_betas, limb_weights, random_sample=True, start_idx=0, max_len=-1):
        ...
        # Better manually cleanup GPU memory when loading motions (Issue #98)
        if "gts" in self.__dict__:
            # First set all GPU tensors to None to release references
            self.gts, self.grs, self.lrs, self.grvs, self.gravs, self.gavs, self.gvs, self.dvs = None, None, None, None, None, None, None, None
            self._motion_lengths, self._motion_fps, self._motion_dt, self._motion_num_frames, self._motion_bodies, self._motion_aa = None, None, None, None, None, None
            if flags.real_traj:
                self.q_gts, self.q_grs, self.q_gavs, self.q_gvs = None, None, None, None

            # Then run garbage collection and clear GPU cache
            gc.collect()
        ...
        if random_sample:
            # ペア単位でサンプリング
            num_pairs = num_motion_to_load // 2
            pair_sampling_prob = self._get_pair_sampling_prob()
            sampled_pair_idxes = torch.multinomial(pair_sampling_prob, num_samples=num_pairs, replacement=True).to(self._device)
            sample_idxes = self._get_paired_motion_idxes(sampled_pair_idxes)
        else:
        ...
        # Cleanup large intermediate data structures to prevent memory leaks (Issue #98)
        del motions, res_acc
        gc.collect()
        torch.cuda.empty_cache()
    ...
    def update_soft_sampling_weight(self, failed_keys):
        ...
        if len(failed_keys) > 0:
            ...
            print("Probalility:", self._sampling_prob[self._sampling_prob.nonzero()].flatten())
            print(f"###############################################################################################################################")
        else:
            all_keys = self._motion_data_keys.tolist()
            self._sampling_prob = torch.ones(self._num_unique_motions).to(self._device) / self._num_unique_motions  # For use in sampling batches

    def update_failed_motion_weight(self, failed_keys, failed_weight_multiplier=2.0):
        """
        失敗したモーションの採択確率を成功したものの指定倍にする weighted sampling
        caregiverとrecipientのペアリングは保持しつつ、ペア単位で重み付け
        """
        if len(failed_keys) > 0:
            # 初期化: すべてのモーションに基本重み 1.0 を設定
            self._sampling_prob = torch.ones(self._num_unique_motions).to(self._device)
            ...
            for k in failed_keys:
                try:
                    ...
                    # ペアの相手も同じ重みを適用
                    base_key = k.replace('_caregiver', '').replace('_recipient', '')
                    for pair_key, pair_dict in self._motion_pairs.items():
                        if pair_key == base_key:
                            if 'caregiver' in pair_dict and 'recipient' in pair_dict:
                                caregiver_idx = pair_dict['caregiver']
                                recipient_idx = pair_dict['recipient']
                                if caregiver_idx not in failed_indexes:
                                    failed_indexes.append(caregiver_idx)
                                if recipient_idx not in failed_indexes:
                                    failed_indexes.append(recipient_idx)
                            break
                except ValueError:
                    print(f"Warning: Failed key '{k}' not found in motion data keys")
                    continue

            # 失敗したモーションの重みを指定倍に設定
            if failed_indexes:
                self._sampling_prob[failed_indexes] = failed_weight_multiplier

                # 正規化して確率分布にする
                self._sampling_prob = self._sampling_prob / self._sampling_prob.sum()

                print("############################################################ Failed Motion Weighted Sampling (Pair-aware) ############################################################")
                print(f"Failed motions weight multiplier: {failed_weight_multiplier}")
                print(f"Number of failed motions: {len(failed_indexes)}")
                print(f"Failed motion probability: {self._sampling_prob[failed_indexes].mean():.6f}")
                print(f"Success motion probability: {self._sampling_prob[~torch.isin(torch.arange(self._num_unique_motions, device=self._device), torch.tensor(failed_indexes, device=self._device))].mean():.6f}")
                print("Failed motion keys:", failed_keys[:10] if len(failed_keys) > 10 else failed_keys)  # Show first 10 or all if <= 10
                print(f"###############################################################################################################################")
            else:
                # 失敗したモーションが見つからない場合は均等分布
                self._sampling_prob = self._sampling_prob / self._sampling_prob.sum()
        else:
            # 失敗したモーションがない場合は均等分布
    ...
    def update_sampling_prob(self, termination_history):
        if len(termination_history) == len(self._termination_history) and termination_history.sum() > 0:
            ...
            return True
        elif len(termination_history) != len(self._termination_history):
            print(f"Termination history length mismatch: expected {len(self._termination_history)}, got {len(termination_history)}")
            # Resize or truncate to match current motion library size
            if len(termination_history) > len(self._termination_history):
                termination_history = termination_history[:len(self._termination_history)]
            else:
                # Pad with zeros if saved history is shorter
                padded_history = torch.zeros_like(self._termination_history)
                padded_history[:len(termination_history)] = termination_history
                termination_history = padded_history
            
            if termination_history.sum() > 0:
                self._sampling_prob[:] = termination_history/termination_history.sum()
                self._termination_history = termination_history
                return True
    ...
    def clear_cache(self):
        # Better manually cleanup GPU memory (Issue #98)
        if "gts" in self.__dict__:
            self.gts, self.grs, self.lrs, self.grvs, self.gravs, self.gavs, self.gvs, self.dvs = None, None, None, None, None, None, None, None
            self._motion_lengths, self._motion_fps, self._motion_dt, self._motion_num_frames, self._motion_bodies, self._motion_aa = None, None, None, None, None, None
        ...
        if "dof_pos" in self.__dict__:
            self.dof_pos = None
        if flags.real_traj and "q_gts" in self.__dict__:
            self.q_gts, self.q_grs, self.q_gavs, self.q_gvs = None, None, None, None

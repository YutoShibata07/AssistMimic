...
from assistmimic.learning.vq_quantizer import EMAVectorQuantizer, Quantizer
from assistmimic.learning.pnn import PNN
...
def load_mcp_mlp(checkpoint, activation = "relu", device = "cpu", mlp_name = "actor_mlp"):
    ...
    # Check if this is a PNN model
    pnn_keys = [k for k in checkpoint['model'].keys() if k.startswith("a2c_network.pnn.actors.0.")]
    if pnn_keys and mlp_name == "actor_mlp":
        # Load from PNN model - use the first actor (index 0)
        key_name = "a2c_network.pnn.actors.0"
        loading_keys = [k for k in checkpoint['model'].keys() if k.startswith(key_name)]
        # PNN models have their own final layer (e.g., actors.0.12), don't add mu
    else:
        # Original behavior for non-PNN models

    ...
    def restore(self, fn):
        ...
        if hasattr(self, 'device') and torch.cuda.is_available():
            if 'cuda' in str(self.device):
                rank = int(str(self.device).split(':')[-1]) if ':' in str(self.device) else 0
        ...
        if torch.cuda.is_available():
            ...
            checkpoint = torch.load(fn, map_location=current_device)
        else:
        ...
        # Load model state dict with strict=False to skip mismatched weights (like discriminator)
        try:
            self.model.load_state_dict(checkpoint['model'], strict=False)
            print("Loaded model weights (partial loading, skipped mismatched weights)")
        except Exception as e:
            print(f"Error loading model weights: {e}")
            # Try to load only actor weights
            actor_state_dict = {}
            for key, value in checkpoint['model'].items():
                if 'disc' not in key and '_disc_' not in key:  # Skip discriminator weights
                    actor_state_dict[key] = value
            
            try:
                self.model.load_state_dict(actor_state_dict, strict=False)
                print("Loaded actor weights only (skipped discriminator)")
            except Exception as e2:
                print(f"Failed to load actor weights: {e2}")
        
        if self._normalize_amp_input:
            try:
                self._amp_input_mean_std.load_state_dict(checkpoint['amp_input_mean_std'])
            except:
                print("Warning: Could not load amp_input_mean_std")

            if self._normalize_input:
                try:
                    self.running_mean_std.load_state_dict(checkpoint['running_mean_std'])
                except:
                    print("Warning: Could not load running_mean_std")
    ...
    def restore(self, fn):
        ...
        if self._normalize_amp_input:
            ...
            if torch.cuda.is_available():
                ...
                checkpoint = torch.load(fn, map_location='cpu')
            # Move checkpoint to CPU to prevent GPU0 memory bias
            if 'amp_input_mean_std' in checkpoint and hasattr(checkpoint['amp_input_mean_std'], 'items'):
                cpu_amp_state = {}
                for key, tensor in checkpoint['amp_input_mean_std'].items():
                    if hasattr(tensor, 'cpu'):
                        cpu_amp_state[key] = tensor.cpu()
                    else:
                        cpu_amp_state[key] = tensor
                checkpoint['amp_input_mean_std'] = cpu_amp_state
            self._amp_input_mean_std.load_state_dict(checkpoint['amp_input_mean_std'])

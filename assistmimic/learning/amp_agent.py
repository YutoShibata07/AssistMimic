from assistmimic.utils.running_mean_std import RunningMeanStd
...
from assistmimic.env.tasks.humanoid_amp_task import HumanoidAMPTask
...
import copy
from assistmimic.utils.torch_utils import project_to_norm
import learning.amp_datasets as amp_datasets
from assistmimic.learning.loss_functions import kl_multi
...
class AMPAgent(common_agent.CommonAgent):
    ...
    def __init__(self, base_name, config):
        ...
        if self.vec_env.env.task.fitting:
            ...
            if hasattr(self, 'ppo_device') and 'cuda' in str(self.ppo_device):
                rank = int(str(self.ppo_device).split(':')[-1])
                # torch.cuda.set_device(rank)  # Moved to run_hydra.py before env init
                # torch.cuda.empty_cache()
            # Load checkpoint with proper device mapping to avoid CUDA device mismatch
            model_path = self.vec_env.env.task.models_path[0]
            ...
            # Move checkpoint tensors to CPU to ensure equal GPU memory distribution
            ...
            # Use set_full_state_weights instead of load_my_state_dict for better shape mismatch handling
            self.set_full_state_weights(checkpoint)  # loads everything (model, std, ect.). that can be load from the last model with shape mismatch handling.
            # self.value_mean_std # not freezing value function though.
        if hasattr(self.vec_env, 'env') and hasattr(self.vec_env.env, 'task') and hasattr(self.vec_env.env.task, 'set_training_agent'):
            try:
                self.vec_env.env.task.set_training_agent(self)
                print("✓ Training agent reference set for validation detection")
            except Exception as e:
                print(f"✗ Failed to set training agent reference: {e}")

        # Setup role-based optimizers if weight_share=False after full initialization
        if hasattr(self, 'setup_role_based_optimizers_if_needed'):
            self.setup_role_based_optimizers_if_needed()
        else:
            # Fallback for inheritance issues - call directly
            self._setup_role_based_optimizers_fallback()
        return
    ...
    def set_full_state_weights(self, weights):
        # Replace mismatched weights with randomly initialized ones of correct shape
        import torch
        filtered_weights = weights.copy()
        if 'model' in filtered_weights:
            model_weights = filtered_weights['model'].copy()
            current_model_dict = self.model.state_dict()
            
            # Create a new dict with only the keys we need
            new_model_weights = {}
            
            for key in current_model_dict.keys():
                if key in model_weights:
                    # Check if shapes match
                    if current_model_dict[key].shape != model_weights[key].shape:
                        print(f"Shape mismatch for {key}: current {current_model_dict[key].shape} vs checkpoint {model_weights[key].shape}")
                        # Create randomly initialized tensor with correct shape
                        correct_shape_tensor = torch.randn_like(current_model_dict[key])
                        new_model_weights[key] = correct_shape_tensor
                        print(f"Replaced {key} with random initialization of shape {correct_shape_tensor.shape}")
                    else:
                        print(f"Successfully loaded {key} with shape {current_model_dict[key].shape}!!")
                        new_model_weights[key] = model_weights[key]
                else:
                    # Key not in checkpoint, use current model's initialization
                    print(f"Key {key} not found in checkpoint, keeping current initialization")
                    # Don't add to new_model_weights, let PyTorch use current initialization
            
            # Remove unexpected keys from checkpoint that don't exist in current model
            unexpected_keys = set(model_weights.keys()) - set(current_model_dict.keys())
            if unexpected_keys:
                print(f"Ignoring unexpected keys in checkpoint: {list(unexpected_keys)[:10]}...")  # Show first 10
            
            filtered_weights['model'] = new_model_weights
        
        # Use strict=False to allow missing keys for new components
        # For the multi-architecture, we handle missing keys by using strict=False
        if 'model' in filtered_weights:
            print("Loading model with strict=False to handle new components...")
            self.model.load_state_dict(filtered_weights['model'], strict=False)
            
            # Load other components normally
            for key in filtered_weights.keys():
                if key != 'model' and hasattr(self, key):
                    try:
                        getattr(self, key).load_state_dict(filtered_weights[key])
                    except:
                        print(f"Could not load {key}, keeping current state")
        else:
            super().set_full_state_weights(filtered_weights)
    ...
    def play_steps_rnn(self):
        ...
        # Ensure both tensors are on the same device for distributed training
    ...
    def play_steps(self):
        ...
        for n in range(self.horizon_length):
            ...
            if len(done_indices) > 0:
                episode_rewards = self.current_rewards.squeeze(-1)  # Remove extra dimension
    ...
    def post_epoch(self, epoch_num):
        ...
        self.running_mean_std_temp.freeze()
        # Rebuild optimizer at epoch 600 to exclude frozen parameters
        if epoch_num == 600:
            print(f"=" * 60)
            print(f"EPOCH {epoch_num}: Rebuilding optimizer without frozen parameters")
            print(f"=" * 60)

            # Get network (handle DDP wrapping)
            network = self.model.a2c_network
            if hasattr(network, 'module'):
                network = network.module

            # Collect trainable parameters (exclude frozen modules)
            trainable_params = []

            for name, param in network.named_parameters():
                # if any(pattern in name for pattern in frozen_param_patterns):
            ...
            print(f"✓ Reduced learning rate by 10x with {len(trainable_params)} trainable parameters")
            print(f"Learning rate (original: {self.last_lr}, reduced: {self.last_lr * 0.1})")
            print(f"=" * 60)

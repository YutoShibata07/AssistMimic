...
import assistmimic.env.tasks.humanoid_amp_task as humanoid_amp_task
from assistmimic.env.tasks.humanoid_amp import HumanoidAMP, remove_base_rot
from assistmimic.utils.motion_lib_smpl import MotionLibSMPL
from assistmimic.utils.motion_lib_base import FixHeightMode
...
class HumanoidIm(humanoid_amp_task.HumanoidAMPTask):
    ...
    def _load_motion(self, motion_train_file, motion_test_file=[]):
        ...
        if self.humanoid_type in ["smpl", "smplh", "smplx"]:
            motion_lib_cfg = EasyDict({
                ...
                "fix_height": FixHeightMode.pene_fix,
    ...
    def _debug_humanoid_weights(self):
        """Debug method to print humanoid weights"""
        if hasattr(self, 'env_role_assignments') and hasattr(self, 'humanoid_handles'):
            for env_id in range(min(4, self.num_envs)):  # Check first 4 envs
                role = getattr(self, 'env_role_assignments', {}).get(env_id, f'env_{env_id}')
                if env_id < len(self.envs) and env_id < len(self.humanoid_handles):
                    ...
                    try:
                        ...
                        total_mass = sum(prop.mass for prop in rigid_props)
                        frame = self.progress_buf[0].item() if len(self.progress_buf) > 0 else 0
                        # print(f"[DEBUG] Frame {frame}: Env {env_id} ({role}) total mass: {total_mass:.3f}kg")
                    except Exception as e:
                        print(f"[DEBUG] Error checking weights for env {env_id}: {e}")
    ...
    def post_physics_step(self):
        ...
        # Ensure amp_obs is computed and available in extras
        if hasattr(self, '_update_hist_amp_obs'):
            self._update_hist_amp_obs()
        if hasattr(self, '_compute_amp_observations'):
    ...
    def step(self, actions):
        ...
        # Debug: Print humanoid weights probabilistically
        import random
        if random.random() < 0.001:  # 0.1% chance per frame
            self._debug_humanoid_weights()
    ...
    def _compute_reset(self):
        ...
        if self.zero_out_far and self.zero_out_far_train:
            ...
            self.reset_buf[:], self._terminate_buf[:] = compute_humanoid_im_reset(  # Humanoid reset
                ...
                flags.no_collision_check, use_mean=True) #flags.im_eval and (not self.strict_eval)
            ...
            self.reset_buf[:], self._terminate_buf[:] = compute_humanoid_im_reset(self.reset_buf, self.progress_buf, self._contact_forces, self._contact_body_ids, \
                                                                               ...
                                                                               self._termination_distances[..., self._reset_bodies_id], flags.no_collision_check, use_mean=True) #flags.im_eval and (not self.strict_eval)
...
@torch.jit.script  
def compute_imitation_reward_with_role(root_pos, root_rot, body_pos, body_rot, body_vel, body_ang_vel, ref_body_pos, ref_body_rot, ref_body_vel, ref_body_ang_vel, rwd_specs, role_weights):
    ...
    # Apply role-specific weights to tracking reward
    base_reward = w_pos * r_body_pos + w_rot * r_body_rot + w_vel * r_vel + w_ang_vel * r_ang_vel
    reward = base_reward * role_weights  # Apply role-specific weighting
    reward_raw = torch.stack([r_body_pos, r_body_rot, r_vel, r_ang_vel], dim=-1)
...
def compute_humanoid_im_reset(reset_buf, progress_buf, contact_buf, contact_body_ids, rigid_body_pos, ref_body_pos, pass_time, enable_early_termination, termination_distance, disableCollision, use_mean):
    ...
    if (enable_early_termination):
        if use_mean:
            # Use per-environment termination distance (support different thresholds for caregiver/recipient)
            mean_distance = torch.norm(rigid_body_pos - ref_body_pos, dim=-1).mean(dim=-1, keepdim=True)  # Shape: (num_envs, 1)
            # termination_distance shape: (num_envs, num_bodies), take first body's threshold per env
            per_env_threshold = termination_distance[:, 0:1]  # Shape: (num_envs, 1)
            has_fallen = torch.any(mean_distance > per_env_threshold, dim=-1)  # using average, same as UHC"s termination condition
        else:

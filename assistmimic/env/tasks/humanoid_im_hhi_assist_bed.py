...
import joblib
from assistmimic.utils.flags import flags
from assistmimic.utils import torch_utils
from assistmimic.utils.torch_utils import quat_to_tan_norm

# Import the parent class
from assistmimic.env.tasks.humanoid_im import HumanoidIm, compute_point_goal_reward

# Import RSI module
from assistmimic.env.tasks.RSI import RSIMixin


class HumanoidAssistBed(HumanoidIm, RSIMixin):
    """
    ...
    def __init__(self, cfg, sim_params, physics_engine, device_type, device_id, headless):
        ...
        self.enable_adjust_caregiver_hand_reference = cfg["env"].get("enable_adjust_caregiver_hand_reference", True)
        ...
        # Build bed-related tensors after all initialization is complete (similar to humanoid_strike)
        ...
        # Per-episode max torque tracking for recipients
        self.episode_max_torques = torch.zeros(num_recipients, device=self.device, dtype=torch.float)
        # Per-step COM positions for stability calculation (recipients only)
        self.episode_com_positions = []  # List of tensors [num_recipients, 3] per step
        # Store body masses for COM calculation (get from recipient humanoid)
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        # Initialize bed heights for all recipient environments
        recipient_env_ids = torch.arange(1, self.num_envs, 2, device=self.device, dtype=torch.long)
        # self._adjust_bed_heights_for_envs(recipient_env_ids)
    ...
    def _setup_tensors(self):
        ...
        # Copy parent _setup_tensors implementation but modify the assertion
        from isaacgym import gymtorch
        
        # Force sensor setup (from parent)
        if self.cfg["env"].get("enableCameraSensors", False):
            if hasattr(self, 'force_sensor_joints') and len(self.force_sensor_joints) > 0:
    ...
    def _setup_character_props(self, key_bodies):
        ...
        # Update self.key_bodies to the filtered version so _key_body_ids is built correctly
        self.key_bodies = humanoid_key_bodies
        
        super()._setup_character_props(humanoid_key_bodies)
        print(f"[DEBUG] AMP obs per step: {self._num_amp_obs_per_step}, key bodies count: {len(humanoid_key_bodies)}")
        print(f"[DEBUG] _dof_obs_size: {self._dof_obs_size}, dof_names length: {len(self._dof_names)}")
    ...
    def _build_bed(self, env_id, env_ptr):
        ...
        if env_id % 2 == 1:
            # env_spacing * 2だけx方向に移動 AARM_JBBC_EDGG
    ...
    def _adjust_bed_heights_for_envs(self, env_ids):
        """Adjust bed heights for specific recipient environments based on their current motion

        Uses motion-based caching to avoid redundant computations for the same motion.
        """
        if not hasattr(self, '_motion_lib') or self._motion_lib is None:
            return

        # Initialize cache for motion-based bed height computations
        if not hasattr(self, '_bed_height_cache'):
            self._bed_height_cache = {}  # motion_id -> (min_z, min_y)

        # Use torch.no_grad() to prevent memory leaks from computational graph
        with torch.no_grad():
            for env_id in env_ids:
                env_id_val = env_id.item() if torch.is_tensor(env_id) else env_id

                # Only adjust for recipient environments (odd env_ids)
                if env_id_val % 2 == 1:
                    # Get current motion ID for this environment
                    if hasattr(self, '_sampled_motion_ids') and env_id_val < len(self._sampled_motion_ids):
                        motion_id = self._sampled_motion_ids[env_id_val].item()

                        # Check cache first to avoid redundant computation
                        if motion_id in self._bed_height_cache:
                            min_z, min_y = self._bed_height_cache[motion_id]
                        else:
                            # Compute bed heights for this motion and cache the result
                            min_z, min_y = self._compute_bed_heights_for_motion(motion_id)
                            self._bed_height_cache[motion_id] = (min_z, min_y)

                        # Adjust bed position for this specific environment
                        if hasattr(self, '_bed_states') and env_id_val < len(self._bed_states):
                            # Set bed height (z) based on min_z for support
                            if min_z < 0.4572:
                                self._bed_states[env_id_val, 2] = min_z - 0.4572 - 0.05

            # Apply updated bed states to simulation, make sure env_ids are lower than num_envs
            if hasattr(self, '_bed_states') and len(env_ids) > 0 and max(env_ids) <= self.num_envs:
                bed_actor_ids = self._bed_actor_ids[env_ids]
                self.gym.set_actor_root_state_tensor_indexed(
                    self.sim,
                    gymtorch.unwrap_tensor(self._root_states),
                    gymtorch.unwrap_tensor(bed_actor_ids),
                    len(bed_actor_ids)
                )

    def _compute_bed_heights_for_motion(self, motion_id):
        """Compute min_z and min_y for a given motion ID

        This is separated out to enable caching and reduce redundant computations.

        Args:
            motion_id: Motion ID to compute bed heights for

        Returns:
            tuple: (min_z, min_y) values for the motion
        """
        with torch.no_grad():
            # Get motion length and sample multiple time points
            motion_length = self._motion_lib.get_motion_length(motion_id)
            num_samples = 50  # Use fewer samples for efficiency during reset
            time_samples = torch.linspace(0, motion_length, num_samples, device=self.device)
            motion_ids_tensor = torch.full((num_samples,), motion_id, dtype=torch.long, device=self.device)

            # Get motion states for all sampled times
            motion_result = self._motion_lib.get_motion_state(motion_ids_tensor, time_samples)

            # Handle different return value formats and gather all body positions
            if isinstance(motion_result, dict):
                if "rg_pos" in motion_result:  # [num_samples, num_bodies, 3]
                    all_body_pos = motion_result["rg_pos"].detach()
                elif "rb_pos" in motion_result:  # fallback key if present
                    all_body_pos = motion_result["rb_pos"].detach()
                elif "root_pos" in motion_result:  # fallback to root only
                    all_body_pos = motion_result["root_pos"].detach().unsqueeze(1)
                else:
                    all_body_pos = None
            elif isinstance(motion_result, (list, tuple)):
                # Fallback: assume first element is root positions [num_samples, 3]
                all_body_pos = motion_result[0].detach().unsqueeze(1)
            else:
                # Unknown type; treat as root-only [num_samples, 3]
                all_body_pos = motion_result.detach().unsqueeze(1)

            if all_body_pos is None:
                # Return default values if no body positions available
                del motion_result, time_samples, motion_ids_tensor
                return (0.0, 0.0)

            # Extract min_z from first 3 samples (use .detach() to avoid graph accumulation)
            body_z_values = all_body_pos[:3, :, 2]
            min_z = body_z_values.min().item()
            del body_z_values  # Clean up immediately

            # Compute min Y excluding elbows and all hand joints (wrists & fingertips)
            # Ensure body name to id mapping is available
            if not hasattr(self, '_body_name_to_id') or len(getattr(self, '_body_name_to_id', {})) == 0:
                if hasattr(self, '_setup_body_name_to_id_mapping'):
                    self._setup_body_name_to_id_mapping()
            ...
            exclude_mask = torch.zeros(num_bodies, dtype=torch.bool, device=self.device)
            body_name_to_id = getattr(self, '_body_name_to_id', {})
            for name in exclude_names:
                if isinstance(body_name_to_id, dict) and name in body_name_to_id:
                    idx = body_name_to_id[name]
                    if isinstance(idx, torch.Tensor):
                        idx = idx.item()
                    if isinstance(idx, int) and 0 <= idx < num_bodies:
                        exclude_mask[idx] = True

            include_mask = ~exclude_mask
            # Extract min_y with filtering (use .detach() to avoid graph accumulation)
            body_y_values = all_body_pos[:, include_mask, 1]
            min_y = body_y_values.min().item()
            del body_y_values  # Clean up immediately

            # Clean up large tensors immediately to prevent memory accumulation
            del all_body_pos, motion_result, time_samples, motion_ids_tensor, exclude_mask, include_mask
    ...
    def _compute_reset(self):
        ...
        # Check ankle heights and terminate if either ankle exceeds 0.6m
    ...
    def _compute_observations(self, env_ids=None):
        ...
        role_labels = (env_ids % 2).float().unsqueeze(1)  # 0 for even (caregiver), 1 for odd (recipient)
    ...
    def _compute_humanoid_obs_v4(self, env_ids):
        ...
        parent_class = super(HumanoidAssistBed, self)
    ...
    def _compute_task_obs_with_role_handling(self, env_ids=None, save_buffer=True):
        ...
        if self.obs_v == 1:
            from assistmimic.env.tasks.humanoid_im import compute_imitation_observations
            ...
            from assistmimic.env.tasks.humanoid_im import compute_imitation_observations_v6
    ...
    def _sample_ref_state(self, env_ids):
        ...
        # Adjust bed heights for reset environments based on their new motion assignments
        # self._adjust_bed_heights_for_envs(env_ids)
    ...
    def _apply_recipient_mass_scaling(self):
        ...
        # print(f"[MASS_SCALING] Test mode: {getattr(__import__('assistmimic.utils.flags'), 'flags', type('', (), {'test': 'UNKNOWN'})).test}")
    ...
    def post_physics_step(self):
        ...
        # # Periodic memory cleanup to prevent OOM and Isaac Gym internal memory leak
        # # Aggressive cleanup for bed collision memory leak issue
        # if hasattr(self, 'progress_buf'):
        #     # Every 10 steps: Python garbage collection
        #     if self.progress_buf[0] % 10 == 0:
        #         import gc
        #         gc.collect()

        #     # Every 50 steps: CUDA cache cleanup
        #     if self.progress_buf[0] % 10 == 0:
    ...
    def _reset_envs(self, env_ids):
        ...
        # Reset bed tensors if they exist (similar to humanoid_strike)
    ...
    def _track_evaluation_metrics(self):
        ...
        All metrics are computed for recipient environments only (odd indices in HHI-assist).
        ...
        if num_recipients == 0:
            raise RuntimeError("No recipient environments found. HHI-assist requires paired environments.")
    ...
    def get_episode_evaluation_metrics(self):
        ...
        Note:
            In im_eval mode, terminated episodes continue physics simulation until batch ends.
            For failed episodes, COM stability includes post-termination states (e.g., fallen).
            Use 'succ' filtered metrics in im_amp_players.py for accurate comparison.
        """
        num_recipients = self.num_envs // 2
        metrics = {
